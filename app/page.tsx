...
async function analyzeImage(formData: FormData) {
  ...
  const indicators = Array.from(new Set(formData.getAll("indicators").map(String).filter(Boolean)));
  ...
  const signals = generateSignals(series, { enabledIndicators: indicators });
  const risk = calculateRisk(series, signals, { useAtr: indicators.includes("atr") });
  const metaPayload: Record<string, unknown> & { indicators: string[] } = {
    ...(meta ?? {}),
    indicators,
  };
  const sourceKind =
    typeof metaPayload.kind === "string"
      ? (metaPayload.kind as string)
      : process.env.OPENAI_API_KEY
        ? "vision"
        : "heuristic";
  const id = saveAnalysis({ series, signals, risk, meta: metaPayload });
  addStatus({
    id: uuid(),
    event: "Analys skapad",
